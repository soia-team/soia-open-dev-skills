...
派发外部 AI CLI 前，把请求写成一页**任务书**。任务书记录已批准规格，不覆盖用户原始要求或授权；发现转述遗漏时以原始授权核对并只暂停受影响动作。本文件给出 8 字段模板、四条写作原则、三段式交付契约、证据有效性优先的 check 0，以及收敛上限。
...
2. **执行者说明实际加载项及必要技能未加载的原因**（例如不可用、加载失败或任务书列错名）。主控按实际执行证据判断影响；只有缺失必要技能导致验收证据不足时补齐，不因节区或措辞不同退回交付。

没有适用技能时写 `applicable_skills: []`，不是任务书缺陷；有必要技能则列名并实际加载。不得为凑数叠加无关流程。
...
**技能 frontmatter 的触发词只是兜底，不能替代本字段。** 触发词写的是动作措辞（「实现这个任务」「修复这个 bug」），真实任务书写的是问题措辞（如「把这份『哪些旧地址归到哪个 rail』的知识改成数据驱动」）；2026-09-12 实测两者零重叠，同日的三臂对照里唯一变量就是本字段——不带它时技能加载数为 0，带上它才实际加载。因此不要把「技能没被加载」的修复预算投在扩触发词变体上（措辞空间随任务无限，且本技能仓的 `scripts/audit_skills.py` 对 frontmatter `description` 有 150 字符硬上限，逐条堆变体会挤占每次会话都要加载的清单预算）。数据与边界见 `failure-modes.md` 的「触发词在真实任务书措辞下的命中情况」。
