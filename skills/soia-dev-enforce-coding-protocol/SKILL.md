...
description: 为工程任务补充范围、权限与验证底线，不另起流程。触发：执行编码协议、按协议约束工程改动、核对工程任务是否越界
version: 1.2.2
...
仅约束工程代码、配置、测试、Git 或行为契约的改动，以及针对它们的诊断/审查。混合任务只对相关部分应用；保持契约不变的纯文字修正、解释和状态汇报走宿主通用流程。不需要先登记模式、加载参考或另交协议报告。
## 短协议
...
每条条款在挡什么失效模式，见[失效模式索引](references/failure-modes.md)；起草或复审任务书时按需加载。守卫的 pass→fail→pass 取证、台账与例外基线见[可证伪守卫](references/falsifiable-guards.md)；设计、清点或复审守卫时按需加载。
