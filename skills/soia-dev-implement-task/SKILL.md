...
description: 实现工程需求、定位修复缺陷或处理已授权 findings。触发：按任务书实现、修复代码缺陷、处理工程审查发现
version: 1.2.2
...
6. **按已授权档位收口。** 用户或项目规定需要独立评审时，自检后发起一次并随交付回报；没有此要求的低风险局部修改完成自核即可。不得仅因调用本技能自动增加独立评审或 panel。审核者只读固定候选，收到目标、范围、验收标准和实现者模型证据；不得由实现者预设审核结论或排除范围内问题。
   同宿主优先使用内置 subagent；只有外部 CLI 调用才加载 `soia-dev-agent-cli-dispatch`。独立性按用户/项目政策，至少排除同一实际模型自审；无政策时采用不同模型家族。实际模型证据缺失如实标未验证，不用请求值代填。额度、费用或权限不满足时报告评审未执行，不静默换模型。
   评审交判断而非合并许可；主控核验关键证据，冲突项并列交裁决。多视角/panel 仅在另有明确要求时运行，不从一次独立评审继续递归派发。

复现路径不清楚时，先按[诊断回路](references/diagnosis-loop.md)建一条能在本次缺陷上变红的回路，再形成假设与修改。

用户或项目要求正式验收线时，读取[验收线](references/acceptance-line.md)并遵循其逐项证据与 Open 回执。普通局部修改直接对照约定行为和实际检查收口，不强制额外表格或四段模板；新增守卫、修复失败路径时仍要证明相关检查能识别错误。
...
- **评审按本次政策执行**：已有授权不逐步重问，未要求的评审不自动追加；没有新候选或新证据不循环审查。提交、合并、发布、部署、安装或扩大修复范围，仍不由“实现完成”自动授权。
- **日志与完成回执：** 结果在前，附关键改动、真实检查与残余风险；本次要求的评审随回报给出结论或未执行原因（独立性缺证据时标 `blocked_independence`），冲突项两方说法并列。finding 输入另列 fix/reject/defer。没有实际运行的检查直说未验证，不凑固定字段。
### 依赖与安装
