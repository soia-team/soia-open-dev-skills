...
```bash
npx skills add soia-team/soia-open-dev-skills -a <agent> -s soia-dev-officecli-ops -y
```
...
安全包装器会设置 `OFFICECLI_SKIP_UPDATE=1`，避免一次文档操作在后台改变 CLI 版本。升级应作为独立任务执行并验证。
...
只使用客户授权的 Office 文件，读取也通过临时副本保护原件。交付写明确输出路径，不上传文件，不在普通配置、日志或技能仓保存客户正文、凭据或会话。临时副本由包装器在结束后清理；用户产物与历史证据不自动删除。
...
8. **“读取”也走临时副本。** 上游在打开特定异常文件时会原地修复 XML 编码或悬空 relationship；为保护源文件，默认通过 `officecli_inspect.py` 查询。
...
需要截图时使用 `--artifact-output <preview.png> -- view screenshot`。属性名、值格式或命令语法不确定时先运行 `officecli help <format> <element> --json`，不凭记忆猜。
