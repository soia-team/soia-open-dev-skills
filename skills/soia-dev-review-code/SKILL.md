...
`soia-dev-implement-task` 第 6 步在交付自检通过后会自动派发一次独立评审，被这样调用时按本节约定执行，不要求调用方另附格式说明。
...
- **输出**：第一行声明形态——双执行体写 `Method: dual-agent (A: <标识> · B: <标识>)`，降级写 `⚠️ DEGRADED: single-context (<原因>)`；随后给「建议通过 / 建议先改 / 建议拒绝」+ 问题清单（位置、触发条件、影响、依据）+ 未验证项。这份文本要能被直接贴进执行者的回报，调用方不需要二次改写。
