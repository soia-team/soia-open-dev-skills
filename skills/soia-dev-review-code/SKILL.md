...
- **输入**：候选标识（commit SHA / 快照摘要或内容指纹）、任务书或验收标准、本次改动范围、被审实现者所用模型标识（供 Independence Gate 当 `executor_model` 对照）。缺候选标识只说明缺口，不审一个会漂移的版本；缺实现者模型标识时独立性按 `blocked_independence` 处理，不写成「已独立复核」。
