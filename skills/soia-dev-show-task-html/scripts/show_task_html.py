...
footer {{ color:var(--muted); text-align:center; margin-top:16px; font-size:12px; }} @media (max-width:700px) {{ main {{ width:min(1320px,calc(100% - 16px)); padding-top:10px; }} h1 {{ font-size:24px; word-break:keep-all; }} .hero,.section {{ padding:14px; border-radius:10px; }} .grid,.compact-grid,.stats {{ grid-template-columns:1fr; }} .progress-row {{ grid-template-columns:minmax(0,1fr) auto; column-gap:12px; }} .progress-owner {{ grid-column:1; grid-row:2; }} .progress-side {{ grid-column:2; grid-row:2; }} th,td {{ min-width:104px; }} }}
