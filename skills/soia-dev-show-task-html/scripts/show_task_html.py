...
* {{ box-sizing:border-box; }} body {{ margin:0; background:var(--bg); color:var(--ink); font:14px/1.6 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; user-select:text; overflow-wrap:break-word; word-break:keep-all; line-break:strict; hanging-punctuation:first last; }}
main {{ width:min(1320px,calc(100% - 24px)); margin:0 auto; padding:20px 0 36px; }} h1,h2,h3 {{ text-wrap:balance; }} h1 {{ margin:0 0 8px; font-size:28px; line-height:1.2; overflow-wrap:anywhere; word-break:keep-all; }} h2 {{ margin:0 0 12px; font-size:20px; line-height:1.2; }} h3 {{ margin:14px 0 8px; font-size:16px; line-height:1.2; }}
.hero {{ background:linear-gradient(135deg,#1e3a8a,#2563c9); color:#fff; border:1px solid #1e4faa; border-radius:12px; padding:18px; box-shadow:0 6px 18px #17255420; }} .objective,.overview {{ margin:0; }} .objective {{ font-size:14px; max-width:92ch; }} .meta {{ display:flex; flex-wrap:wrap; gap:8px; margin-top:14px; }} .meta span {{ border:1px solid #ffffff4d; border-radius:999px; padding:3px 8px; font-size:12px; line-height:1.4; }}
.section {{ background:var(--panel); border:1px solid var(--line); border-radius:12px; padding:14px; margin-top:10px; }} .grid,.compact-grid {{ display:grid; gap:12px; align-items:stretch; }} .grid {{ grid-template-columns:repeat(auto-fit,minmax(min(100%,230px),1fr)); }} .card {{ display:flex; flex-direction:column; border:1px solid var(--line); border-radius:10px; padding:12px; background:#fbfcfe; min-width:0; }} .card-title {{ font-size:16px; font-weight:700; line-height:1.2; text-wrap:balance; }} .card-detail {{ color:var(--muted); }} .card-detail:empty {{ display:none; }}
.stats {{ display:grid; grid-template-columns:repeat(auto-fit,minmax(180px,1fr)); gap:12px; align-items:stretch; }} .stat {{ display:flex; flex-direction:column; align-items:stretch; min-width:0; min-height:92px; border:1px solid var(--line); border-radius:10px; padding:12px; background:#fbfcfe; }} .stat strong {{ display:block; font-size:20px; line-height:1.2; overflow-wrap:break-word; word-break:keep-all; }} .stat span {{ display:block; margin-top:auto; overflow:hidden; color:var(--muted); font-size:12px; line-height:1.4; text-overflow:ellipsis; white-space:nowrap; }} .stat-done,.stat-pass {{ border-color:#a9dec4; }} .stat-active {{ border-color:#a9c4fa; }} .stat-blocked,.stat-fail {{ border-color:#f3b4af; }}
.progress-list {{ border-top:1px solid var(--line); }} .progress-row {{ display:grid; grid-template-columns:minmax(0,1fr) clamp(120px,9vw,180px) auto; column-gap:20px; row-gap:6px; align-items:start; padding:14px 8px; border-bottom:1px solid var(--line); }} .progress-row:nth-child(even) {{ background:#f8fafc; }} .progress-main {{ min-width:0; }} .progress-main strong,.progress-main span {{ display:block; }} .progress-main strong {{ font-size:16px; line-height:1.2; }} .progress-main span,.progress-owner,.progress-next {{ color:var(--muted); font-size:12px; }} .progress-meta {{ display:contents; }} .progress-owner {{ align-self:start; min-width:0; }} .progress-side {{ display:grid; gap:6px; align-items:start; justify-items:start; min-width:0; }} .progress-row .status {{ align-self:center; justify-self:start; margin:0; }} .compact-grid {{ grid-template-columns:repeat(auto-fit,minmax(min(100%,190px),1fr)); }}
.status,.claim {{ display:inline-block; margin:6px 6px 0 0; border-radius:999px; padding:2px 7px; font-size:12px; font-weight:700; line-height:1.3; white-space:nowrap; }} .status-done,.status-pass,.claim-observed {{ color:var(--good); background:#e7f6ee; }} .status-active,.claim-inferred {{ color:var(--accent); background:#e8f0ff; }} .status-blocked,.status-fail {{ color:var(--bad); background:#feebea; }} .status-pending,.status-unknown,.claim-unknown {{ color:var(--warn); background:#fff4d6; }}
.evidence-meta {{ margin-top:5px; color:var(--muted); }} .ref {{ display:inline-block; margin:4px 5px 0 0; padding:1px 5px; border:1px solid var(--line); border-radius:4px; background:#f7f9fc; }} .ref.missing {{ font-size:12px; }} code {{ font:12px ui-monospace,SFMono-Regular,Menlo,monospace; }}
...
footer {{ color:var(--muted); text-align:center; margin-top:16px; font-size:12px; }} @media (max-width:700px) {{ main {{ width:min(1320px,calc(100% - 16px)); padding-top:10px; }} h1 {{ font-size:24px; word-break:keep-all; }} .hero,.section {{ padding:14px; border-radius:10px; }} .grid,.compact-grid,.stats {{ grid-template-columns:1fr; }} .stat {{ min-height:62px; padding:8px 10px; }} .stat strong {{ font-size:18px; }} .stat span {{ margin-top:2px; }} .progress-row {{ grid-template-columns:minmax(0,1fr); column-gap:0; row-gap:8px; }} .progress-main span,.progress-next {{ overflow-wrap:break-word; word-break:normal; }} .progress-meta {{ display:flex; flex-wrap:wrap; gap:6px 8px; align-items:center; min-width:0; }} .progress-owner {{ align-self:center; }} .progress-side {{ display:flex; flex-wrap:wrap; gap:6px; align-items:center; min-width:0; }} th,td {{ min-width:104px; }} }}
